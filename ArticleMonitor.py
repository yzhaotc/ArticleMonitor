...
def monitor(conn,infotype,sday,eday):
    ...
    if infotype=='webhoset100':
        try:
            cur.execute("SELECT COUNT(*) from sentiment.t100articles WHERE publish_timestamp between '%s' and '%s' "%(sday,eday)+"and article_source like '%Web%';")
            caocao=cur.fetchall()
        except:
            print('webhoset100 not found')
    if infotype=='webhoset600':
        try:
            cur.execute("SELECT COUNT(*) FROM sentiment.t600sentiment_tracker WHERE fail_at between '%s' and '%s' "%(sday,eday)+" and s3_url like '%webhose%';")
            caocao=cur.fetchall()
        except:
    ...
    if infotype=='webhosenomapping':
        try:
            cur.execute("SELECT COUNT(*) FROM sentiment.t600sentiment_tracker WHERE fail_at between '%s' and '%s' "%(sday,eday)+" and s3_url like '%webhose%' and valid_fail_reason like '%No mapping%';")
            caocao=cur.fetchall()
        except:
            print('Webhose no mapping not found')
    if infotype=='webhosenoentity':
        try:
            cur.execute("SELECT COUNT(*) FROM sentiment.t600sentiment_tracker WHERE fail_at between '%s' and '%s' "%(sday,eday)+" and s3_url like '%webhose%' and valid_fail_reason like '%No entity%';")
            caocao=cur.fetchall()
        except:
    ...
    if infotype=='webhosenoprocessor':
        try:
            cur.execute("SELECT COUNT(*) FROM sentiment.t600sentiment_tracker WHERE fail_at between '%s' and '%s' "%(sday,eday)+" and s3_url like '%webhose%' and fail_reason is not null';")
            caocao=cur.fetchall()
        except:
...
Final_DF.to_excel('MonitorJuly.xlsx')
